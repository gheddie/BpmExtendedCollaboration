...
public class M7CompletementListener implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		...
		ProcessInstance matchingAnotherSlave = BusinessKeyMatcher
				.forProcessDefinitionKey(ProcessConstants.AnotherSlave.DEF.DEF_ANOTHER_SLAVE_PROCESS)
				.withParentBusinessKey(parentBusinessKey)
				.withAdditionalValue(item.getValue()).singleResult(runtimeService);
